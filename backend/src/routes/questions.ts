...
const buildPersonalContext = async (
  ...
  return [
    subjectMemory
      ? contextSnippet(
          `${subjectName} student memory map`,
          `Risk: ${subjectMemory.riskLevel}
...
Recent topics: ${JSON.stringify(subjectMemory.recentTopics)}`
    ...
    learningSignals
      ...
      .map((signal) =>
        contextSnippet(
          `${subjectName} learning signal: ${signal.signalType}`,
          `${signal.topic ?? "General"} - ${signal.title}. ${signal.detail} Evidence: ${signal.evidence}. Next: ${signal.nextAction ?? ""}`
        )
      )
      .join("\n\n"),
