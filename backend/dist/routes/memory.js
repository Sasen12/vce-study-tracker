...
memoryRouter.get("/events", asyncHandler(async (req, res) => {
    const authReq = req;
...
memoryRouter.get("/signals", asyncHandler(async (req, res) => {
    const authReq = req;
    const query = listQuerySchema.parse(req.query);
...
memoryRouter.post("/student-map/rebuild", asyncHandler(async (req, res) => {
    const authReq = req;
    const subjectMemories = await rebuildStudentMemoryMaps(authReq.user.id);
    res.json({ subjectMemories });
}));
