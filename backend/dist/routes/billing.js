...
import { billingPlanIds, billingProfile, cancelSubscriptionAccess, createCheckoutSession, createCustomerPortalSession, getBillingUser, isPaidPlan, normalizeBillingPlan, stripe, syncCheckoutSession, syncSubscription, updateUserBillingPlan } from "../services/billingService.js";
...
const returnUrlFor = (req, returnUrl) => returnUrl ?? process.env.FRONTEND_URL ?? req.get("origin") ?? "http://localhost:8081";
...
billingRouter.post("/admin/users/:id/plan", asyncHandler(async (req, res) => {
    const authReq = req;
    ...
    try {
        switch (event.type) {
            ...
            case "customer.subscription.deleted":
                await cancelSubscriptionAccess(event.data.object);
                break;
            default:
                break;
        }
