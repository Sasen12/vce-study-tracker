...
const buildWeeklySubjectSquads = async (viewerUserId) => {
    ...
    return COMMUNITY_SQUADS.map((squad) => {
        ...
        const openQuestionCount = wall.filter((question) => squadForSubjectName(question.subjectName)?.id === squad.id && question.answerCount === 0).length;
...
const buildCommunityPulse = (squads, liveRooms, questionWall) => {
