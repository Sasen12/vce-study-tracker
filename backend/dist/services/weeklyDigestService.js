...
const hmac = (value) => crypto.createHmac("sha256", digestSecret()).update(value).digest("base64url");
export const createWeeklyDigestUnsubscribeToken = (user) => {
...
export const verifyWeeklyDigestUnsubscribeToken = async (token) => {
    let parsed;
    try {
    ...
    if (!parsed.userId || !parsed.signature)
        return null;
...
export const unsubscribeWeeklyDigest = async (token) => {
...
const loadDigestUsers = async (options, range) => prisma.user.findMany({
    where: {
...
const formatShortDate = (date) => new Intl.DateTimeFormat("en-AU", {
    timeZone: APP_TIME_ZONE,
    weekday: "short",
    day: "numeric",
    month: "short"
}).format(date);
const formatWeekLabel = (range) => `${formatShortDate(range.weekStart)} - ${formatShortDate(new Date(range.weekEnd.getTime() - 1))}`;
const subjectBreakdown = (user) => {
...
const strongestLine = (user) => {
    const topSubject = subjectBreakdown(user)[0];
    if (topSubject)
        return `${topSubject.name}: ${minutes(topSubject.seconds)} minutes logged`;
    if (user.subjects[0])
        return `${user.subjects[0].subjectName}: ready for the first block`;
    return "Add subjects so Forge can build a sharper weekly plan.";
};
const nextMove = (user) => {
    const firstSignal = user.learningSignals[0];
    if (firstSignal?.nextAction)
        return firstSignal.nextAction;
    if (firstSignal)
        return `Repair ${firstSignal.subjectName}: ${firstSignal.title}.`;
    const nextEvent = user.events[0];
    if (nextEvent)
        return `Start with ${nextEvent.subject?.subjectName || nextEvent.eventType}: ${nextEvent.title}.`;
    const topSubject = subjectBreakdown(user)[0];
    if (topSubject)
        return `Repeat the strongest habit: one focused block for ${topSubject.name}.`;
    return "Log one 25-minute block so next week has evidence to work from.";
};
const buildDigest = (user, range) => {
...
export const sendWeeklyDigestToAllUsers = async (options = {}) => {
