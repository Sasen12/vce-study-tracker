...
export const STARTER_TITLE_IDS = [DEFAULT_TITLE_ID, "year_11_rookie", "year_12_rookie"];
...
export const levelFromXp = (xp) => LEVELS.reduce((best, level) => (xp >= level.xp ? level.level : best), 1);
...
const mergeBadges = (current, incoming) => Array.from(new Set([...badgesAsArray(current), ...incoming]));
const mergeCosmetics = (current, incoming) => Array.from(new Set([
    DEFAULT_THEME_ID,
    ...STARTER_TITLE_IDS.map((titleId) => `title:${titleId}`),
    ...cosmeticsAsArray(current),
    ...incoming
]));
const starterTitleIds = new Set(STARTER_TITLE_IDS);
const isStarterTitle = (titleId) => starterTitleIds.has(titleId);
export const inferStarterTitleFromSubjectCounts = (counts) => {
    if (counts.unit12Count > counts.unit34Count)
        return "year_11_rookie";
    if (counts.unit34Count > counts.unit12Count)
        return "year_12_rookie";
...
export const ensureGamification = async (userId) => {
    ...
    const streakExpired = gamification.currentStreak > 0 && (!lastStudyDate || (lastStudyDate !== today && !isYesterday(lastStudyDate, today)));
...
export const applyTheme = async (userId, themeId) => {
    const theme = themeById(themeId);
    if (!theme)
        throw new Error("Theme not found");
    const gamification = await ensureGamification(userId);
...
export const grantThemeToUser = async (userId, themeId, equip = true) => {
    const theme = themeById(themeId);
    if (!theme)
        throw new Error("Theme not found");
    const gamification = await ensureGamification(userId);
...
export const applyTitle = async (userId, titleId) => {
    const title = titleById(titleId);
    if (!title)
        throw new Error("Title not found");
    const gamification = await ensureGamification(userId);
...
export const awardQuestionBadges = async (userId) => {
...
export const awardFoundingBadges = async (userId) => {
...
export const syncAllBadges = async (userId) => {
