...
const graphVisualTypes = new Set(["line_graph", "scatter_plot", "bar_chart"]);
const graphImagesEnabled = () => process.env.OPENAI_GRAPH_IMAGES_ENABLED?.trim().toLowerCase() !== "false";
const graphImageModel = () => process.env.OPENAI_GRAPH_IMAGE_MODEL?.trim() || process.env.OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const graphImageLimit = (fallback) => {
...
const visualDataSummary = (visual) => {
...
const graphVisualHasData = (visual) => graphVisualTypes.has(visual.type) &&
    ((visual.type === "bar_chart" && visual.bars.length >= 2) ||
        ((visual.type === "line_graph" || visual.type === "scatter_plot") && visual.points.length >= 2));
const buildGraphImagePrompt = (visual, contextLabel) => `Create a clean educational VCE graph image for ${contextLabel}.
...
const addImageToGraphVisual = async (openai, visual, contextLabel) => {
    if (!graphVisualHasData(visual))
        return visual;
...
const addImagesToVisuals = async (openai, visuals, contextLabel, limit) => {
    if (!graphImagesEnabled() || limit <= 0)
        return visuals;
    let remaining = limit;
    const enhanced = [];
...
const addImagesToGeneratedQuestions = async (openai, input, questions) => {
    ...
    if (!graphImagesEnabled() || remaining <= 0)
        return questions;
    const enhanced = [];
...
const buildStudyAnswerPrompt = (input) => {
    ...
    const visualPromptUseful = Boolean(visualProfile?.visual) || /\b(graph|chart|plot|diagram|visual|sketch|map|data|function|relationship)\b/i.test(input.question);
...
const sanitiseStudyAnswer = (answer) => ({
    ...answer,
...
const addImagesToStudyAnswer = async (openai, input, answer) => {
    const visuals = await addImagesToVisuals(openai, answer.visuals ?? [], `${input.subject ?? "VCE"} coach answer`, graphImageLimit(1));
