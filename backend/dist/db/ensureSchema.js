...
const ensureCommunityTrustSchema = async () => {
  ...
  `);
    await prisma.$executeRawUnsafe("CREATE INDEX IF NOT EXISTS community_question_helpful_votes_answer_idx ON community_question_helpful_votes(answer_message_id)");
    await prisma.$executeRawUnsafe(`
  ...
  `);
    ...
    await prisma.$executeRawUnsafe("CREATE INDEX IF NOT EXISTS community_reports_reported_user_created_idx ON community_reports(reported_user_id, created_at)");
