import { normaliseSubjectName, subjectMetaFor, vceSubjectCatalogue } from "./vceSubjectCatalogue.js";
...
const hasPhrase = (question, phrase) => new RegExp(`(^|[^a-z0-9])${escapeRegExp(phrase)}([^a-z0-9]|$)`, "i").test(question);
const importantWords = (subject) => normaliseSubjectName(subject)
    .replace(/[:/,-]/g, " ")
    .split(/\s+/)
    .filter((word) => word.length > 2 && !stopWords.has(word));
const generatedAliasesFor = (subjectName) => {
    ...
    return Array.from(new Set([
        ...
        .filter((alias) => Boolean(alias && alias.length >= 2))));
};
const scorePatterns = (question, signals = []) => signals.reduce((score, signal) => score + (signal.pattern.test(question) ? signal.weight : 0), 0);
const scoreAliases = (question, aliases) => aliases.reduce((score, alias) => {
    if (!hasPhrase(question, alias))
        return score;
    return score + Math.min(14, 5 + importantWords(alias).length * 2);
}, 0);
export const inferVceSubjectFromQuestion = (question) => {
