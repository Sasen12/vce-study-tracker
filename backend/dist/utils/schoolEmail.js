...
const titleCaseWord = (word) => (word ? `${word[0].toUpperCase()}${word.slice(1).toLowerCase()}` : word);
const formatSchoolDomainLabel = (label) => label
    .replace(/[-_]+/g, " ")
    ...
    .map(titleCaseWord)
    .join(" ");
export const isVicEduAuEmail = (email) => {
    const domain = email.trim().toLowerCase().split("@")[1] ?? "";
    return domain === "vic.edu.au" || domain.endsWith(vicEduAuSuffix);
};
export const inferSchoolNameFromEmail = (email) => {
    const domain = email.trim().toLowerCase().split("@")[1] ?? "";
    if (!domain.endsWith(vicEduAuSuffix))
        return null;
